...
`,
      ...
      document: {
        ...
        restoreConfirmContent: '这将基于所选内容创建新版本并重新解析，是否继续？',
        alreadyCurrent: '已是当前版本',
        restoreSuccess: '已恢复，正在重新解析',
        currentBadge: '当前',
        originUpload: '上传',
        originRestore: '恢复',
        version: '版本',
...
<p>编译后的知识单元以分块形式合并进分块流输出，适合在分块文档之上构建可检索的知识层。</p>`,
      ...
      clinical: `<p>支持的文件格式为<b>DOCX</b>、<b>PDF</b>、<b>TXT</b>。</p><p>
      临床指南和参考文献等临床文档具有分层的章节结构。 我们使用文本特征来检测章节和建议（推荐）标题作为分割点。
      </p><p>
      每条编号建议都会作为独立的块保留，并包含其所在的章节路径，以免丢失上层上下文。
      </p>`,
...
示例：general/v2/`,
    ...
    fileManager: {
      ...
      conflictDialogTitle: '文档已存在',
      conflictDialogDescription: '以下文件在该数据集中已存在，请选择每个文件的处理方式。',
      conflictReplace: '替换',
      conflictReplaceDescription: '保存为新版本并重新解析',
      conflictKeepBoth: '两者保留',
      conflictKeepBothDescription: '以自动重命名的名称上传',
      conflictRemove: '移除',
      conflictResolve: '确认',
    },
    flow: {
